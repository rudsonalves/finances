...
class BalanceStore implements BalanceStorer {
  ...
  ///
  /// Returns the row ID of the newly inserted balance, or -1 if an error
  /// occurs.
  @override
  ...
  ///
  /// Returns a map representing the balance's data if found, or null if not
  /// found.
  @override
  ...
  /// and date, or null if no record is found.
  @override
  Future<Map<String, dynamic>?> queryBalanceInDate({
  ...
  /// Queries the all balances record before a specified date for a given
  /// account.
  ///
  /// This method searches for the all balances record for the specified
  /// account that is dated before the given date. It's useful for retrieving
  /// the known balances state before a certain point in time.
  ///
  /// Parameters:
  ///   - account: The account ID for which the balance is being queried.
  ///   - date: The date before which the balance record is sought, represented
  ///           as an integer (e.g., milliseconds since epoch).
  ///
  /// Returns:
  ///   A map representing the all balance's data before the specified date, or
  ///   null if no such record exists. The map includes balance details such as
  ///   balance ID, account ID, balance date, opening balance, closing balance,
  ///   etc.
  ///
  /// Throws:
  ///   An exception if there's an error during the database query, logging the
  ///   error.
  ///
  /// Note:
  ///   This method is particularly useful for financial calculations that
  ///   require understanding the account balance before a specific transaction
  ///   or event date.
  @override
  Future<List<Map<String, dynamic>>> queryAllBalanceAfterDate({
    required int account,
